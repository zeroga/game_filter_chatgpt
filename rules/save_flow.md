...
每期生成阶段严格只读。周报必须用唯一 `report_period` 附带完整的“建议写入内容”，列出推荐清单、候选状态、拟更新 `last_snapshot`、`report_generated`、`report_written` 及其他拟落库字段的当前值、建议值和目标逻辑键，并全部标记“尚未写入”。

用户阅读后可全部确认、部分确认或拒绝。只有该期已经展示且被当次明确确认的项目才能执行；部分确认只写确认项，拒绝不写入，全部确认也不得扩展到未展示项目。推荐或候选状态写入仍须执行场景分层写入清单并逐项回查。

`last_snapshot` 只表示最近一次经用户确认保存的周报快照。只有相关业务写入成功且用户当次确认包含快照时才更新；未确认、未确认快照或写入失败时保留旧值。`report_generated` 和 `report_written` 如需落库也必须列入当次确认范围，不是自动写入例外。创建订阅或 Automation 的确认不得解释为以后每期的持续写入授权。
